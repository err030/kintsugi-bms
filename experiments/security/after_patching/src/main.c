...
static volatile uint32_t g_progress_flags;
static volatile uint32_t g_task_heartbeat;
static volatile uint32_t g_attack_heartbeat;
...
static void bms_protection_task(void *pv) {
    ...
    for (;;) {
        ...
        if (g_bms_faults.ovp || g_bms_faults.uvp || g_bms_faults.ocp || g_bms_faults.otp || g_bms_faults.utp) {
...
static void bms_comm_task(void *pv) {
    ...
    for (;;) {
        printf("[BMS][COMM] TX frame: V=%.1f SoC=%.1f%% SoH=%.1f%% FAULT=%d\r\n",
               ...
               g_bms_state.soc,
               g_bms_state.soh,
               (int)(g_bms_faults.ovp || g_bms_faults.uvp || g_bms_faults.ocp || g_bms_faults.otp || g_bms_faults.utp));
...
TaskHandle_t hotpatch_attack_task_handle;
...
int main(void)
{
    g_boot_flag = 0xA5A5A5A5;
    g_progress_flags = 0;
    g_progress_flags |= (1u << 0);
    kintsugi_enable_cycle_counter();

    hp_manager_init();
    g_progress_flags |= (1u << 1);

    hp_mpu_init();
    g_progress_flags |= (1u << 2);
    ...
    *(uint32_t *)((uint32_t)&hotpatch_code + sizeof(struct hp_header) + 4) = (uint32_t)(&target_ram_function + 16) | 1;
    ...
    printf("Task BMS_CALIB_ATTACK: %d\r\n", xTaskCreate(bms_calibration_attack_task, "BMS_CALIB", configMINIMAL_STACK_SIZE, NULL, 5, &calib_attack_task_handle));
#endif
#if ENABLE_HOTPATCH_CODE_ATTACK
    printf("Task HP_CODE_ATTACK: %d\r\n", xTaskCreate(vulnerable_task, "HP_ATTACK", configMINIMAL_STACK_SIZE, NULL, 5, &hotpatch_attack_task_handle));
#endif
