...
static const char __attribute__((aligned(8))) invalid_certificate[] = 
    "-----BEGIN CERTIFICATE-----\n"
...
static void main_task(void * pcCertificate) {
    uint32_t result;
    ...
    while (true) {
        ...
        result = mbedtls_x509_crt_parse( &xCertContext, ( const uint8_t * ) invalid_certificate, strlen( invalid_certificate ) + 1 );
        if( result == 0 )
        {
            printf("Imported Successfully\r\n");
            mbedtls_x509_crt_free( &xCertContext );
        }
        else if (result == MBEDTLS_ERR_ECP_FEATURE_UNAVAILABLE) {
            printf("Import Failed -> Hotpatch Succeeded!\r\n");
        }
        else {
            printf("Import Failed -> Other Error: %08X\r\n", result);
        }
        vTaskDelay(1000);
    }
}
