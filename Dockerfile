...
# Unpack the nRF SDK
WORKDIR ${SDK_DIR}
COPY nRF5_SDK_17.1.0_ddde560.zip .
...
# ---- venv + west + pin revision, THEN install deps
RUN     python3 -m venv ${RTOS_DIR}/zephyr/zephyrproject/.venv \
    &&  . ${RTOS_DIR}/zephyr/zephyrproject/.venv/bin/activate \
    &&  python3 -m pip install -U "pip<25" setuptools wheel \
    ...
    &&  cd ${RTOS_DIR}/zephyr/zephyrproject \
    &&  west -v update \
    &&  printf "six==1.16.0\nsh==1.14.3\nsly==0.4\nsetuptools-scm==7.1.0\n" > /tmp/zephyr-pip-constraints.txt \
    &&  pip install -c /tmp/zephyr-pip-constraints.txt -r zephyr/scripts/requirements.txt \
    &&  if [ -f modules/hal/espressif/zephyr/requirements.txt ]; then \
            pip install -c /tmp/zephyr-pip-constraints.txt -r modules/hal/espressif/zephyr/requirements.txt ; \
        fi \
    &&  if [ -f bootloader/mcuboot/zephyr/requirements.txt ]; then \
            pip install -c /tmp/zephyr-pip-constraints.txt -r bootloader/mcuboot/zephyr/requirements.txt ; \
    fi \
    &&  if [ -f modules/lib/nanopb/extra/requirements.txt ]; then \
            pip install -c /tmp/zephyr-pip-constraints.txt -r modules/lib/nanopb/extra/requirements.txt ; \
        fi \
    &&  cd ${RTOS_DIR}/zephyr/zephyrproject/zephyr \
...
# --- 关键环境变量：让 CMake/West 自动找到 Zephyr SDK
ENV ZEPHYR_TOOLCHAIN_VARIANT=zephyr \
    ZEPHYR_SDK_INSTALL_DIR=${SDK_DIR}/zephyr-sdk-0.16.8

# （可选）有些脚本离开 west 环境时也能找到 Zephyr 根目录
ENV ZEPHYR_BASE=${RTOS_DIR}/zephyr/zephyrproject/zephyr
